...
function ProductUpdate() {
  ...
  // Gallery images state: array of File objects (new) and existing URLs
  ...
  useEffect(() => {
    ...
    apiInstance
      .get(`vendor/product/${vendorId}/${productId}/`)
      .then((res) => {
        const p = res.data;
        setTitle(p?.title || "");
        ...
        setShippingAmount(p?.shipping_amount ?? "");
        ...
        if (p?.specification && Array.isArray(p.specification) && p.specification.length > 0) {
          setSpecifications(
            p.specification.map((s) => ({
              title: s.title || "",
              content: s.content || "",
            }))
          );
        } else {
          setSpecifications([{ title: "", content: "" }]);
        }
        ...
        if (p?.color && Array.isArray(p.color) && p.color.length > 0) {
          setColors(
            p.color.map((c) => ({
              name: c.name || "",
          ...
          setColors([{ name: "", color_code: "#000000" }]);
        }

        // Load existing sizes
        if (p?.size && Array.isArray(p.size) && p.size.length > 0) {
          setSizes(
            p.size.map((s) => ({
              name: s.name || "",
      ...
      .catch((e) => {
        console.error(e);
        Swal.fire({ icon: "error", text: "Could not load product" });
      })
      .finally(() => setLoadingProduct(false));
  }, [vendorId, productId, userId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append("oldPrice", oldPrice || "0");
      fd.append("shipping_amount", shippingAmount || "0");
      fd.append("stock_qty", stockQty || "0");
      ...
      // Always send specifications field, even if empty array
      ...
      // Always send colors field, even if empty array
      ...
      // Always send sizes field, even if empty array
      fd.append("sizes", JSON.stringify(cleanedSizes));

      await apiInstance.patch(`vendor/product/${vendorId}/${productId}/`, fd, {
      ...
      Swal.fire({
        icon: "error",
        text: err?.response?.data?.detail || "Could not update product",
      });
    } finally {
      setLoading(false);
    }
    ...
    <VendorLayout>
      <main className="mb-5">
        <div className="container px-4">
          <h3 className="mb-3">
            <i className="fas fa-edit text-primary" /> Update Product
          </h3>
                          ...
                          <div className="mb-2">
                            ...
                            <div className="d-flex flex-wrap gap-2">
                              {existingGalleryImages.map((img, index) => (
                                <div key={img.id || index} className="position-relative">
                                  <img
                                    src={img.image}
                                    alt={`Gallery ${index + 1}`}
                                    style={{
